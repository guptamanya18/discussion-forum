...
│       │       ├── main.py
│       │       ├── database.py
│       │       ├── kafka_producer.py   # Publishes user-events to Kafka
│       │       ├── core/
...
│       │       │   └── report_routes.py # Thread reporting
│       │       └── services/
│       │           └── like_service.py
│       │
...
│       │       ├── database.py
│       │       ├── kafka_producer.py
│       │       ├── kafka_consumer.py  # Consumes user-events from Kafka
│       │       ├── core/
│       │       ├── models/
│       │       │   ├── user.py
│       │       │   ├── community.py
...
└── frontend/                         # React Single-Page Application
    ...
    └── src/
        ...
        ├── App.js                    # React Router: 16 routes with auth guards
        ├── App.css
        ...
        └── pages/
            ...
            ├── Profile.js
            ├── Dashboard.js
            ├── AdminPanel.js           # Admin-only: user stats, role management, user deletion
            ├── ModeratorPanel.js       # Moderator-only: read-only user listing + stats
            ├── MemberDashboard.js      # Member activity dashboard
            ├── Notifications.js
...
| **AdminPanel** | Admin-only: user stats, role management, user deletion, toggle active/inactive. |
| **ModeratorPanel** | Moderator-only: read-only user listing + stats. |
| **MemberDashboard** | Member activity dashboard. |
...
| bob | bob@example.com | pass123 | moderator (promoted by seed script) |
| charlie | charlie@example.com | pass123 | member |
| diana | diana@example.com | pass123 | member |
| eve | eve@example.com | pass123 | member |
| frank – tina | `<name>@example.com` | pass123 | member (15 more users) |
...
|---|---|
| `Docs/HLD/` | High-Level Design diagrams (system architecture, request flow, etc.) |
| `Docs/LLD/` | Low-Level Design diagrams (class diagrams, sequence diagrams, etc.) |
| `Docs/ER Diagram.png` | Entity-Relationship diagram for all five databases |
...
1. Register 20 users (alice through tina)
2. Login all users
3. Promote bob to moderator (via admin API)
4. Create 8 communities (Python Developers, Web Dev Hub, Data Science Lab, DevOps Corner, Open Source Cafe, Mobile App Builders, Cloud Architecture, Cybersecurity Hub)
5. Random users join communities (8–14 members each)
6. Create 20 threads across communities
7. Like random threads (3–8 likes each)
8. Create 4–7 comments per thread
9. Reply to 20 random comments (with ~40% @mentions)
10. Like random comments
11. Some users leave communities (triggers Kafka events)
